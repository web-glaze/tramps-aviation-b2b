...
export default function B2BLoginPage() {
  ...
  // If the user is already signed in when they hit /b2b/login (e.g. they
  ...
  useEffect(() => {
    if (_hasHydrated && isAuthenticated && user?.role === "agent") {
      ...
      window.location.href = kycApproved ? "/b2b/dashboard" : "/b2b/kyc";
    }
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // commit, the middleware sees no token and bounces us back to /b2b/login
      // (which is exactly the bug we were seeing: toast fires, header shows
      // "Go to Dashboard", but the URL stays on /b2b/login). A real navigation
            ...
            <form onSubmit={handleLogin} className="space-y-4">
              ...
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <Link href="/b2b/forgot-password" className="text-xs text-primary hover:underline">Forgot password?</Link>
                </div>
